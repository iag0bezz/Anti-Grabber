...
if (!options.TestMode)
{
    ...
    Console.Error.WriteLine($"Cenários disponíveis: {string.Join(", ", scenarios.Keys)}, {TlsParserSelfTestScenario}, {ReassemblySelfTestScenario}, {QuicCryptoSelfTestScenario} (sem rede)");
    return 2;
}
...
// código compila. pnOffset=18 é fixo aqui (calculado manualmente a partir do
// header esperado) — o parser do header QUIC que descobre esse valor sozinho é
// uma etapa futura, ainda não existe.
static int RunQuicCryptoSelfTest()
{
    var dcid = Convert.FromHexString("8394c8f03e515708");

    var expectedHeader = Convert.FromHexString("c300000001088394c8f03e5157080000449e00000002");
    ...
    var plaintext = QuicInitialCrypto.TryDecrypt(encryptedPacket, pnOffset: 18, keys);
